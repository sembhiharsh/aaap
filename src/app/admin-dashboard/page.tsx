...
/* ─── Types ─── */
type Section = "reservations" | "new" | "confirmed" | "completed" | "cancelled" | "deleted" | "accounts" | "range" | "invoice" | "drivers" | "totals";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const dQ = query(collection(db, "drivers"), orderBy("createdAt", "desc"));
    const unsubscribeDrivers = onSnapshot(dQ, (snap) => {
      setDrivers(snap.docs.map(d => ({ id: d.id, ...d.data() } as Driver)));
    }, (err) => console.error("Drivers onSnapshot error:", err));
  ...
  const renderSection = () => {
    ...
    if (section === "reservations") return <ReservationsSection bookings={bookings} loading={loading} onRefresh={() => { }} setBookings={setBookings} defaultFilter="ALL" drivers={drivers} />;
    if (section === "website") return <ReservationsSection bookings={bookings} loading={loading} onRefresh={() => { }} setBookings={setBookings} sourceFilter="website" defaultFilter="ALL" drivers={drivers} />;
    ...
    if (section === "drivers") return <DriversSection drivers={drivers} bookings={bookings} db={db} />;
    if (section === "totals") return <DriverTotalsSection drivers={drivers} bookings={bookings} />;
  };
...
function DriversSection({ drivers, bookings, db }: { drivers: Driver[]; bookings: Booking[]; db: any }) {
  ...
  const openEdit = (d: Driver) => {
    ...
    setPin(d.pin);
    setPhone(d.phone || "");
    setStatus(d.status === "Deleted" ? "Disabled" : d.status);
    setIsModalOpen(true);
  };
    ...
    try {
      if (editDriver) {
        await updateDoc(doc(db, "drivers", editDriver.id), {
          name: name.toUpperCase(), pin, phone, status
        });
      } else {
        ...
        await runTransaction(db, async (transaction: any) => {
          ...
          const driverId = `d1DRV${currentId.toString().padStart(3, '0')}`;

          transaction.set(counterRef, { currentId });
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this driver? They will lose access immediately.")) return;
    try {
      await updateDoc(doc(db, "drivers", id), { status: "Deleted" });
    } catch (err) {
      console.error(err);
  ...
  const activeDrivers = drivers.filter(d => d.status !== "Deleted");

  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto w-full animate-fade-in">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black text-[#111111] tracking-tight">Drivers</h1>
          <p className="text-gray-500 mt-1">Manage your team and their access.</p>
        </div>
        <button
          onClick={openAdd}
          className="flex items-center gap-2 px-5 py-2.5 bg-[#111111] text-white rounded-xl font-medium hover:bg-black transition-all shadow-sm"
        >
          <PlusCircle size={20} />
      ...
      {/* Mobile & Fold Card List (No Horizontal Scrolling) */}
      <div className="xl:hidden space-y-3">
        {activeDrivers.length === 0 && (
          <div className="bg-white rounded-2xl p-8 text-center text-gray-400 font-bold border border-gray-100">
            No drivers found.
          </div>
        )}
        {activeDrivers.map((d) => (
          <div key={d.id} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-black text-gray-900 text-[16px]">{d.name}</div>
                
              </div>
              <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${
                d.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
              }`}>
                <div className={`w-1.5 h-1.5 rounded-full ${d.status === 'Active' ? 'bg-green-500' : 'bg-red-500'}`} />
                {d.status}
              </span>
            </div>
            <div className="pt-2 border-t border-gray-100 flex items-center justify-between gap-1 text-[11px] font-bold">
              <button onClick={() => openEdit(d)} className="px-2.5 py-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors">Edit</button>
              <button onClick={async () => {
                if (window.confirm(`Reset PIN for ${d.name}?`)) {
                  const newPin = Math.floor(1000 + Math.random() * 9000).toString();
                  await updateDoc(doc(db, "drivers", d.id), { pin: newPin });
                  alert(`New PIN for ${d.name} is: ${newPin}`);
                }
              }} className="px-2.5 py-1.5 bg-orange-50 text-orange-600 rounded-lg hover:bg-orange-100 transition-colors">Reset PIN</button>
              <button onClick={async () => {
                const newStatus = d.status === "Active" ? "Disabled" : "Active";
                if (window.confirm(`Mark driver as ${newStatus}?`)) {
                  await updateDoc(doc(db, "drivers", d.id), { status: newStatus });
                }
              }} className="px-2.5 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">{d.status === "Active" ? "Disable" : "Enable"}</button>
              <button onClick={() => handleDelete(d.id)} className="px-2.5 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors">Delete</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="hidden xl:block bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-[#F8F9FA] text-[#111111] font-semibold text-[13px] uppercase tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-gray-100">
              {activeDrivers.map((d) => (
                <tr key={d.id} className="hover:bg-[#F8F9FA]/50 transition-colors">
                  <td className="px-6 py-4 font-black text-gray-900 text-[15px]">{d.name}</td>
                  ...
                  <td className="px-6 py-4 text-right space-x-3">
                    <button onClick={() => openEdit(d)} className="text-blue-600 hover:text-blue-800 font-bold">Edit</button>
                    <button onClick={async () => {
                      if (window.confirm(`Reset PIN for ${d.name}?`)) {
                        const newPin = Math.floor(1000 + Math.random() * 9000).toString();
                        await updateDoc(doc(db, "drivers", d.id), { pin: newPin });
                    ...
                    }} className="text-orange-600 hover:text-orange-800 font-bold">Reset PIN</button>
                    <button onClick={async () => {
                      const newStatus = d.status === "Active" ? "Disabled" : "Active";
                      if (window.confirm(`Mark driver as ${newStatus}?`)) {
                        await updateDoc(doc(db, "drivers", d.id), { status: newStatus });
                      }
                    }} className="text-gray-600 hover:text-gray-800 font-bold">{d.status === "Active" ? "Disable" : "Enable"}</button>
                    <button onClick={() => handleDelete(d.id)} className="text-red-600 hover:text-red-800 font-bold">Delete</button>
                  </td>
                </tr>
              ))}
              {activeDrivers.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                    <p className="mb-4 font-bold">No drivers found.</p>
                    <button
                      onClick={openAdd}
                      className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#111111] text-white rounded-xl font-medium hover:bg-black transition-all shadow-sm mx-auto"
                    >
                      <PlusCircle size={20} />
                      <span>Add Driver</span>
                    </button>
      ...
      {isModalOpen && typeof window !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md max-h-[85vh] flex flex-col overflow-hidden shadow-2xl animate-scale-in relative border border-gray-100 my-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0 bg-white sticky top-0 z-10">
              <div>
                <h2 className="text-lg font-black text-gray-900">{editDriver ? "Edit Driver" : "Add Driver"}</h2>
                <p className="text-xs text-gray-500 font-medium mt-0.5">Manage team access & 4-digit PIN</p>
              </div>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 overflow-y-auto flex-1">
              <div>
                ...
                <input required type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 bg-[#F8F9FA] border border-gray-200 rounded-xl text-[15px] font-medium text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20 focus:border-[#8B4513] transition-all" placeholder="John Doe" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">4-Digit PIN *</label>
                <input required type="text" pattern="\d{4}" maxLength={4} value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, ''))} className="w-full px-4 py-3 bg-[#F8F9FA] border border-gray-200 rounded-xl text-[15px] font-medium text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20 focus:border-[#8B4513] transition-all font-mono tracking-[0.3em]" placeholder="1234" />
                <p className="text-xs text-gray-500 mt-1 font-medium">Used by the driver to log in.</p>
              </div>
              {editDriver && (
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Status</label>
                  <select value={status} onChange={(e) => setStatus(e.target.value as any)} className="w-full px-4 py-3 bg-[#F8F9FA] border border-gray-200 rounded-xl text-[15px] font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20 focus:border-[#8B4513] transition-all appearance-none">
              ...
              <div className="pt-2">
                <button disabled={saving} type="submit" className="w-full bg-[#8B4513] text-white py-3.5 rounded-xl font-bold text-[15px] hover:bg-[#8B4513]/90 transition-colors disabled:opacity-50 shadow-sm">
...
/* ═══════════════════════════════════════════════
   DRIVER TOTALS & EARNINGS SECTION
═══════════════════════════════════════════════ */
function DriverTotalsSection({ drivers, bookings }: { drivers: Driver[]; bookings: Booking[] }) {
  ...
  // Filter non-cancelled, non-deleted bookings by date preset
  const validBookings = bookings.filter(b => {
    const s = (b.status || "").toLowerCase();
    if (s === "cancelled" || s === "deleted") return false;

    if (preset === "all") return true;

    const bDate = parseDateForSort(b.date);
    if (!bDate) return false;
    const bTime = new Date(bDate);
    ...
    if (preset === "today") {
      return bTime.getTime() === today.getTime();
    }
    if (preset === "week") {
      ...
      return bTime >= weekAgo && bTime <= nextWeek;
    }
    if (preset === "month") {
      return bTime.getFullYear() === now.getFullYear() && bTime.getMonth() === now.getMonth();
    }
    if (preset === "custom") {
      if (startDate) {
        const start = new Date(startDate);
        start.setHours(0, 0, 0, 0);
        if (bTime < start) return false;
      }
      if (endDate) {
        const end = new Date(endDate);
        end.setHours(23, 59, 59, 999);
        if (bTime > end) return false;
      }
      return true;
    }
    return true;
  ...
  // Collect all driver names from drivers table + any bookings assigned to drivers
  const allDriverNames = Array.from(new Set([
    ...activeDrivers.map(d => d.name.toUpperCase()),
    ...validBookings.map(b => (b.driver || "").trim().toUpperCase()).filter(Boolean)
  ])).sort();

  // Aggregate stats per driver
  const driverStats = allDriverNames.map(name => {
    const driverInfo = activeDrivers.find(d => d.name.toUpperCase() === name);
    const driverRides = validBookings.filter(b => (b.driver || "").trim().toUpperCase() === name);
    const totalDriverEarned = driverRides.reduce((sum, b) => sum + parseNum(b.driverPrice), 0);
    const totalCustomerPaid = driverRides.reduce((sum, b) => sum + parseNum(b.price), 0);
    const netProfit = totalCustomerPaid - totalDriverEarned;
    const completedCount = driverRides.filter(b => (b.status || "").toLowerCase() === "completed").length;
    ...
    const s = search.toLowerCase();
    return d.name.toLowerCase().includes(s) || d.driverId.toLowerCase().includes(s);
  }).sort((a, b) => b.totalDriverEarned - a.totalDriverEarned);

  // Overall totals across all displayed drivers
  const grandTotalDriverEarned = driverStats.reduce((sum, d) => sum + d.totalDriverEarned, 0);
  const grandTotalCustomerPaid = driverStats.reduce((sum, d) => sum + d.totalCustomerPaid, 0);
  const grandTotalProfit = grandTotalCustomerPaid - grandTotalDriverEarned;
  const grandTotalRides = driverStats.reduce((sum, d) => sum + d.rideCount, 0);

  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto w-full animate-fade-in font-sans">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight flex items-center gap-2">
            ...
            Driver Totals & Earnings
          </h1>
        </div>

        {/* Date Filter Presets */}
        <div className="flex flex-wrap items-center gap-1.5 bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm text-xs font-bold">
          <button
            onClick={() => setPreset("all")}
            className={`px-3 py-1.5 rounded-lg transition-all ${preset === "all" ? "bg-[#8B4513] text-white shadow-sm" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}
          >
            All Time
          </button>
          <button
            onClick={() => setPreset("today")}
            className={`px-3 py-1.5 rounded-lg transition-all ${preset === "today" ? "bg-[#8B4513] text-white shadow-sm" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}
          >
            Today
          </button>
          <button
            onClick={() => setPreset("week")}
            className={`px-3 py-1.5 rounded-lg transition-all ${preset === "week" ? "bg-[#8B4513] text-white shadow-sm" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}
          >
            Last 7 Days
          </button>
          <button
            onClick={() => setPreset("month")}
            className={`px-3 py-1.5 rounded-lg transition-all ${preset === "month" ? "bg-[#8B4513] text-white shadow-sm" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}
          >
            This Month
          </button>
          <button
            onClick={() => setPreset("custom")}
            className={`px-3 py-1.5 rounded-lg transition-all ${preset === "custom" ? "bg-[#8B4513] text-white shadow-sm" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}
          >
            Custom
          </button>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs font-medium"
            />
          </div>
          <div className="flex items-center gap-2">
      ...
      {/* Overview Stat Card - Driver Payouts Only */}
      <div className="mb-6">
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200/80 p-4 rounded-2xl shadow-sm inline-flex items-center gap-4 min-w-[260px]">
          <div className="w-12 h-12 rounded-xl bg-amber-500/20 flex items-center justify-center text-amber-800 shrink-0">
            <DollarSign size={24} strokeWidth={2.5} />
          </div>
          <div>
            <span className="text-[11px] font-bold text-amber-900 uppercase tracking-wider block">Driver Payouts</span>
            <p className="text-2xl font-black text-gray-900 leading-tight">€{grandTotalDriverEarned.toFixed(2)}</p>
            <p className="text-[11px] text-amber-700 mt-0.5 font-medium">Total owed to drivers</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative w-full mb-6">
        ...
        <input
          ...
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20 focus:border-[#8B4513] shadow-sm transition-all"
        />
      </div>
      ...
      <div className="space-y-3">
        {driverStats.length === 0 && (
          <div className="bg-white rounded-2xl p-12 text-center text-gray-400 font-bold border border-gray-100 shadow-sm">
            No drivers or bookings found for this period.
          </div>
        )}
        ...
        {driverStats.map((d) => {
          ...
          return (
            ...
            >
              {/* Main Driver Row */}
              <div
                onClick={() => setExpandedDriver(isExpanded ? null : d.name)}
                className="p-4 md:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer hover:bg-gray-50/70 transition-colors"
              >
                {/* Driver Info */}
                <div className="flex items-center gap-3.5">
                  <div className="w-11 h-11 rounded-xl bg-[#8B4513]/10 border border-[#8B4513]/20 flex items-center justify-center text-[#8B4513] font-black text-sm shrink-0">
                    {d.name.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-black text-gray-900 text-base">{d.name}</span>
                      {d.driverId !== "-" && (
                        <span className="px-2 py-0.5 rounded-md bg-gray-100 text-gray-600 font-mono text-[10px] font-bold">
                          {d.driverId}
                        </span>
                      )}
                ...
                <div className="flex items-center justify-between md:justify-end gap-6 pt-3 md:pt-0 border-t md:border-t-0 border-gray-100">
                  <div className="text-left md:text-right">
                    <span className="text-[10px] uppercase tracking-wider font-bold text-gray-400 block">Driver Payout</span>
                    <span className="text-xl font-black text-amber-700">€{d.totalDriverEarned.toFixed(2)}</span>
                  </div>

                  <button
                    type="button"
                    className="flex items-center gap-1 text-xs font-bold text-gray-500 hover:text-gray-900 px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors shrink-0"
                  >
                    <span>{isExpanded ? "Hide Rides" : `View Rides (${d.rideCount})`}</span>
                    {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                  </button>
                </div>
              </div>

              {/* Expandable Rides Breakdown */}
              {isExpanded && (
                <div className="border-t border-gray-100 bg-gray-50/60 p-4 md:p-6">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">
                    Bookings for {d.name} ({d.rides.length})
                  </h4>

                  {d.rides.length === 0 ? (
                    <p className="text-xs text-gray-400 font-medium py-2">No rides recorded in this time range.</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-xs text-gray-600 bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                        ...
                        <tbody className="divide-y divide-gray-150">
                          {d.rides.map((r) => {
                            const dp = parseNum(r.driverPrice);

                            return (
                              <tr key={r.id} className="hover:bg-amber-50/30 transition-colors">
                                <td className="px-4 py-3 whitespace-nowrap font-medium text-gray-900">
                                  {r.date} <span className="text-gray-400 ml-1 font-mono">{r.time}</span>
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap font-mono font-bold text-gray-800">
                                  {r.bookingId}
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap font-medium text-gray-900">
                                  {r.customerName}
                                ...
                                <td className="px-4 py-3 text-gray-600 max-w-[280px] truncate">
                                  <span>{cleanLocation(r.pickup)}</span>
                                  <span className="text-gray-400 mx-1.5">→</span>
                                  <span>{cleanLocation(r.dropoff)}</span>
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap text-right font-black text-amber-700">
                                  €{dp.toFixed(2)}
                                </td>
